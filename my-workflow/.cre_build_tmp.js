...
var onHttpTrigger = (runtime2, triggerOutput) => {
  ...
  runtime2.log(`[snapshot] Received request for token: ${token}`);
  const feedAddress = runtime2.config.feeds[token];
  if (!feedAddress) {
    const supported = Object.keys(runtime2.config.feeds).join(", ");
    throw new Error(`Unsupported token "${token}". Supported: ${supported}`);
  }
  runtime2.log(`[snapshot] Using feed: ${feedAddress}`);
  ...
  const chainSelector = network286.chainSelector.selector;
  const evmClient = new ClientCapability(chainSelector);
  const latestRoundDataCalldata = encodeFunctionData({
    abi: aggregatorABI,
    functionName: "latestRoundData"
  });
  runtime2.log("[snapshot] Calling latestRoundData() on the Data Feed…");
  ...
  runtime2.log(`[snapshot] Price (raw 8-dec int): ${price}`);
  runtime2.log(`[snapshot] updatedAt timestamp:   ${updatedAt}`);
  const latestHeader = evmClient.headerByNumber(runtime2, {}).result();
  const latestBlockNum = latestHeader.header?.blockNumber ? protoBigIntToBigint(latestHeader.header.blockNumber) : 0n;
  runtime2.log(`[snapshot] Latest block: ${latestBlockNum}`);
  const updatedAtBlock = findBlockByTimestamp(runtime2, evmClient, latestBlockNum, updatedAt);
  runtime2.log(`[snapshot] Feed last-updated block: ${updatedAtBlock}`);
  const reportPayload = encodeAbiParameters(parseAbiParameters("string token, uint256 price, uint256 blockNumber, uint256 timestamp"), [token, price, updatedAtBlock, updatedAt]);
  runtime2.log("[snapshot] Generating signed consensus report…");
  ...
  runtime2.log("[snapshot] Submitting report to SnapshotRecorder…");
  const writeResult = evmClient.writeReport(runtime2, {
    receiver: runtime2.config.snapshotRecorderAddress,
  ...
  runtime2.log(`[snapshot] Tx hash: ${txHash}`);
  runtime2.log(`[snapshot] https://sepolia.etherscan.io/tx/${txHash}`);
  return {
    token,
...
function findBlockByTimestamp(runtime2, evmClient, latestBlock, targetTimestamp) {
  if (latestBlock === 0n)
    return 0n;
  ...
  while (lo < hi) {
    ...
    const header = evmClient.headerByNumber(runtime2, { blockNumber: bigintToProtoBigInt(mid) }).result();
    const ts = header.header?.timestamp ?? 0n;
